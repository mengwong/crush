...
func (s *service) Save(ctx context.Context, session Session) (Session, error) {
	todosJSON, err := marshallTodos(session.Todos)
	if err != nil {
		return Session{}, err
	}
...
func (s service) fromDBItem(item db.Session) Session {
	todos, _ := unmarshallTodos(item.Todos.String)
	return Session{
		ID:               item.ID,
...
func marshallTodos(todos []Todo) (string, error) {
	if len(todos) == 0 {
		return "", nil
	}
...
func unmarshallTodos(data string) ([]Todo, error) {
	if data == "" {
		return []Todo{}, nil
	}
	var todos []Todo
